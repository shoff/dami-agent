...
| Capability registry | in progress | Core registry, semantic retrieval, bundle expansion, and native discovery/execution are implemented. F3a adds the official MCP C# client behind a Dami-owned lifecycle, explicit server identity/trust, real protocol discovery, isolated schema snapshots, and loopback-only default transport. F3b secure untrusted-description ingestion is claimed; LocalOnly filtering, MCP dispatch, Host proof, and skills remain. |

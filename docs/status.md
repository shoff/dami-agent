...
| Capability registry | in progress | Core registry, semantic retrieval, bundle expansion, and native discovery/execution are implemented. F3a/F3b add the official MCP client, explicit trust, discovery/schema snapshots, stable identities, and fail-closed summaries. F3c1 threads route privacy through selection; F3c2 adds exact-owner native/MCP dispatch and real SDK invocation. F3c3a/ADR-0015 now provide the scoped, Egressable-only, allowlisted, budgeted, bounded, event-metered request-body HTTP gate. F3c3b provenance and authorized SDK construction are claimed. Host proof and skills remain. |

...
| Capability registry | F1–F4 and F5a/F5b done | Native, MCP, and filesystem skills share one catalog. Skills load progressively and have a complete free lifecycle. The live `propose-tool` capability stages trace-owned source/tests/rationale without execution or registration; bounded metadata list and exact localhost inspection surfaces are deployed. Human single-resolution promotion, verification, and live activation remain F5c. |

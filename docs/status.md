...
| Capability registry | F1–F4, F5a/F5b, and F5c1 done | Native, MCP, and filesystem skills share one catalog. Skills load progressively and have a complete free lifecycle. The live `propose-tool` capability stages trace-owned source/tests/rationale without execution or registration; bounded metadata list and exact localhost inspection surfaces are deployed. The version-pinned promotion request and single-resolution approval ledger is deployed; sandboxed verification and live activation remain F5c2/F5c3. |

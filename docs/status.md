...
| Capability registry | in progress | Core registry, semantic retrieval, bundle expansion, and native discovery/execution are implemented. F3a/F3b add the official MCP client, explicit server trust, real discovery/schema snapshots, stable source-neutral identities, trusted verbatim metadata, and fail-closed local summaries. F3c1 threads route privacy through selection. F3c2 adds exact-owner native/MCP dispatch, real SDK invocation, cancellation, bounded result/error translation, and executable registration during discovery. F3c3's ADR-backed body-capable D-012 remote transport is claimed. Host proof and skills remain. |

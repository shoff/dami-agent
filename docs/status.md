...
| Capability registry | F1–F4, F5a/F5b, F5c1/F5c2/F5c3a/F5c3b done | Native, MCP, filesystem skills, and dynamically activated sandboxed tools share source-neutral catalog and execution seams. Skills load progressively and have a complete free lifecycle. The live `propose-tool` capability stages trace-owned source/tests/rationale without execution or registration; bounded metadata list and exact localhost inspection surfaces are deployed. Exact promotion requests and the fixed package-free verifier are implemented; conforming tests and invocation have run inside the memory/process/time/output-bounded systemd+bubblewrap boundary with no home, repository, network, or persistent writable mount. Verification records the tested assembly digest and evidence before promotion. Registry publication is failure-atomic and verifies the registered assembly before each launch. Approved durable activation rows converge into immutable version-addressed artifacts and idempotent handler/schema/search publication before Host readiness, including restart republication of already-activated tools. The deployed Host has its private runtime root configured and reported a successful 0/0 recovery before listening. The live human promotion remains F5c3c. |

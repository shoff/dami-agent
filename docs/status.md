...
| Capability registry | F1–F4 and F5a done | Native, MCP, and filesystem skills share one catalog. Skills load progressively and can be authored, atomically revised, retired, recovered, and invoked through one native lifecycle capability. Self-authored tool artifacts now have a bounded, version-pinned, append-only staging contract paired transactionally with `ToolProposed`; staging remains inert. Native propose/list/inspect and human-gated activation remain F5b/F5c. |

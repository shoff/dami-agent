...
| Capability registry | in progress | Core registry, semantic retrieval, bundle expansion, and native discovery/execution are implemented. F3a/F3b add the official MCP client, explicit trust, discovery/schema snapshots, stable identities, and fail-closed summaries. F3c is complete: route privacy and trace origin reach every tool execution; remote SDK connect/discovery/invoke/shutdown run in explicit operation scopes; and only the dedicated marked MCP egress handler can construct the remote transport. The ordinary factory remains loopback-only. F3d Host composition/fake-server proof is claimed; skills remain. |

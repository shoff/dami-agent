...
| Capability registry | in progress | Core registry, semantic retrieval, bundle expansion, and native discovery/execution are implemented. F3a/F3b add the official MCP client, explicit server trust, real discovery/schema snapshots, stable source-neutral identities, trusted verbatim metadata, and fail-closed local summaries. F3c1 now threads the route privacy class through the runtime and excludes untrusted MCP both before LocalOnly reranking and during related expansion. F3c2 source-neutral invocation is claimed; an ADR-backed body-capable D-012 remote transport follows. Host proof and skills remain. |

...
| Capability registry | in progress | `Dami.Capabilities` defines the normalized entry model and focused registration/catalog abstractions. Stable-ID lookup, duplicate rejection, immutable metadata, tool/skill reference invariants, and cycle-safe deduplicating bundle expansion have 18 tests. Semantic retrieval, native discovery, MCP, and skill loading remain. |
...
`dotnet test Dami.sln` executed 251 tests across nine suites with 0 failures; preceding

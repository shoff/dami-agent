...
| Capability registry | in progress | Core registry, semantic retrieval, bundle expansion, and native discovery/execution are implemented. F3 is split; F3a MCP client lifecycle, explicit server trust, discovery, and schema caching are claimed. Secure untrusted-description ingestion, LocalOnly filtering, MCP dispatch, Host proof, and skills remain. |

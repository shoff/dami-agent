...
| Capability registry | F1–F4, F5a/F5b, and F5c1/F5c2 done | Native, MCP, and filesystem skills share one catalog. Skills load progressively and have a complete free lifecycle. The live `propose-tool` capability stages trace-owned source/tests/rationale without execution or registration; bounded metadata list and exact localhost inspection surfaces are deployed. Exact promotion requests and the fixed package-free verifier are implemented; conforming tests and invocation have run inside the memory/process/time/output-bounded systemd+bubblewrap boundary with no home, repository, network, or persistent writable mount. Failure-atomic activation/recovery and the live human promotion remain F5c3. |

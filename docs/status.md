...
| Capability registry | in progress | Core registry, semantic retrieval, bundle expansion, and native discovery/execution are implemented. F3a/F3b add the official MCP client, explicit server trust, real discovery/schema snapshots, stable source-neutral identities, trusted verbatim metadata, and fail-closed local summaries. F3c is split: F3c1 LocalOnly selection filtering is claimed; invocation dispatch and an ADR-backed body-capable D-012 remote transport follow. Host proof and skills remain. |

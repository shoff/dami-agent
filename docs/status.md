...
| Capability registry | in progress | Core registry, semantic retrieval, bundle expansion, and native discovery/execution are implemented. F3a/F3b add the official MCP client, explicit server trust, real discovery/schema snapshots, stable source-neutral identities, trusted verbatim metadata, and fail-closed local summaries that keep raw untrusted tool descriptions out of retrieval metadata and advertised tool descriptions. F3c LocalOnly filtering and MCP dispatch are claimed; Host proof and skills remain. |

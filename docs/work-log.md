...
## 2026-09-01 — Codex — Discord media and response-latency diagnosis started

Claimed board item `f95a8966` (M1c) to diagnose why the Dami Discord gateway does not
understand inbound images, create/deliver outbound images, show a typing indicator, or
publish answer fragments as they arrive. Scope is read-only diagnosis and a test-first
remediation plan so this investigation does not overlap Claude's active M1 implementation.

Started by reading `docs/onboarding.md`, the open PostgreSQL task board, the current
Discord host/gateway sources and tests, and the newly landed subscription streaming work.
Next action: clone the current upstream `NousResearch/hermes-agent` into `/tmp`, compare
its Discord media/typing/streaming paths with Dami's, and record file-and-line evidence.

Completed the comparison in `docs/discord-hermes-comparison.md` against upstream Hermes
commit `c56f8cdd4817fcc6a50d79ca9e57b1c173850e4a`. Read-only live checks used
`systemctl status dami-host`, narrow `journalctl -u dami-host` windows, `docker ps`,
`docker logs dami-llm`, `nvidia-smi`, and Ollama's `/api/tags` endpoint.

Observed, rather than inferred: the 12:27 Discord image downloaded in 476 ms; vision then
took 212.6 s. Ollama encoded 4,064 image prompt tokens into a 4,096-token context, evicted
and reloaded models, then reset after a later 126 s local-model request. Frontier
preparation and local fallback both failed and the Discord worker sent no reply. The
text-delay cause is independently structural: Discord calls only completed-response APIs
and its transport exposes neither typing nor message editing, while the streaming frontier
API already present in Core goes unused. Image generation is registered only in the
proactive host; Discord's multipart attachment rail exists but nothing interactive feeds
generated bytes into it.

No production code, service configuration, deployment, or service state was changed.
Verification: `git diff --check` (recorded below after it runs). No test suite was run for
this documentation-only diagnosis.

Verification finished: `git diff --check` passed with no whitespace errors. The only
working-tree changes from this session are this log entry and
`docs/discord-hermes-comparison.md`.

## 2026-09-01 — Codex — Discord responsive multimodal implementation started

Steve approved implementing the remediation from the Discord/Hermes comparison. Added
and claimed board item `ec8ea1a3` (M1d). Planned four strict-TDD increments: typing
lifecycle; rate-limited progressive response delivery over the existing augmented stream;
bounded vision preprocessing/timeouts with visible degradation; and focused interactive
image generation through the existing generator plus attachment rail. Claude still owns
the broader M1 item, so this work is confined to those diagnosed behaviors and will
preserve unrelated concurrent changes.

Implemented M1d in four strict-TDD increments. Each behavior test was run first and
observed failing before its production change: missing `PostTypingAsync`; no typing call
before the model; missing create-message ID support; no progressive create/update path;
no intermediate edit after 80 accumulated characters; no vision timeout option; no
interactive generator dependency; natural-language image requests bypassing generation;
and no typing refresh during a delayed turn. Production changes add a Discord typing
lease, progressive channel create/edit operations behind the existing disclosure policy,
`DiscordReplyStreamer`, a per-image vision cancellation budget, and
`DiscordImageResponder` over the existing audited `IImageGenerator`.

The affected-suite gate initially caught two integration omissions rather than being
reported green: the Discord composition test lacked its new image-generator stub, and
architecture tests rejected the two intentional new egress holders. Added the stub and
pinned `DiscordImageResponder` as the only new channel/image-generator holder, explicitly
noting it has no profile-store dependency.

Verification completed:

- `Dami.Gateway.Discord.Tests`: 59 passed, 0 failed.
- `Dami.Host.Discord.Tests`: 64 passed, 0 failed.
- `Dami.Architecture.Tests`: 18 passed, 0 failed.
- `dotnet build Dami/Dami.sln --no-restore -m:1 --disable-build-servers`: 0 warnings,
  0 errors.
- `dotnet test Dami/Dami.sln --no-restore -m:1 --disable-build-servers`: all 21 test
  assemblies passed, 1,529 tests total, 0 failed.
- `git diff --check`: passed.

No package, project, deployment, service configuration, restart, commit, or push was
performed. The running Discord gateway still uses the previously deployed binaries.
## 2026-09-01 — Codex — G17 GUI subscription multimodal chat (started)

- Claimed board task `cc7f9ecb` to make direct GUI chat always use the subscription-backed augmented frontier, accept image attachments through local vision, and render generated images.
- Inspected `MainWindow.Streaming.cs`, `RuntimeClient.cs`, and `TurnEndpoints.cs`. Observed that the GUI's `FrontierToggle` value is serialized directly as `augmented`; its false state therefore invokes `ITurnRunner.BeginStreamingAsync` and the local LLM. The GUI has no image request or image message contract.
- Plan: drive each behavior with a narrow failing test, remove the direct-chat local-routing choice, add a bounded image attachment contract whose raw bytes go only to loopback vision, and add a generated-image endpoint/rendering path using the existing `IImageGenerator` registration.

## 2026-09-01 — Codex — G17 GUI subscription multimodal chat (completed)

- Added `DirectChatRequestTests.SerializesEveryDirectMessageAsAugmented`; observed the expected compile failure because `DirectChatRequest` did not exist, then added the fixed subscription-only request contract. Removed `FrontierToggle` and the boolean routing argument from the GUI send/stream path. Narrow result: 1/1 passed.
- Added `TurnImageContextTests.DescribesAttachmentLocallyAndLabelsDerivedContext`; observed the expected compile failure because the image context seam and contract did not exist, then added a 12 MiB image contract, local `IVisionClient` description, and disclosure-gated augmented-stream context. Narrow result: 1/1 passed.
- Added GUI image-request serialization coverage; observed the expected compile failure for the absent attachment property, then added the Avalonia image picker, attachment state, loopback serialization, and visible attachment label. Narrow GUI request suite: 2/2 passed.
- Added `ImageGenerationPromptTests` and `InteractiveImageGeneratorTests`; observed the expected compile failures for both absent production types, then added explicit generation intent recognition, `/images/generate`, the existing governed `IImageGenerator` adapter, and inline Avalonia bitmap rendering. Narrow results: 3/3 GUI prompt cases and 1/1 Host generator test passed.
- Verification: `dotnet build Dami/Dami.sln --no-restore` succeeded with 0 warnings and 0 errors; GUI suite passed 102/102; Host suite passed 88/88; `dotnet test Dami/Dami.sln --no-build --no-restore` passed all 1,593 tests across 21 assemblies; `git diff --check` produced no errors.
- No service was restarted and no GUI build was installed, so live subscription, local-vision, and provider-backed generation demonstrations remain deployment work rather than claimed evidence.
## 2026-09-01 — Codex — G18 GUI drag/drop and clipboard image input (started)

- Steve rejected the visible attachment picker from G17. Added and claimed board task `4787258b` to replace it with image-file drag/drop and Ctrl+V clipboard image ingestion while retaining the existing localhost-only vision contract and 12 MiB host validation.
- Inspected Avalonia 12.1's installed API metadata: drag/drop exposes `IAsyncDataTransfer.TryGetFilesAsync`; the clipboard exposes `TryGetFilesAsync` and `TryGetBitmapAsync`; `Bitmap.Save(Stream)` provides a PNG encoding path for raw clipboard pixels.

## 2026-09-01 — Codex — G18 GUI drag/drop and clipboard image input (completed)

- Added `ChatImageInputTests`; observed the expected compile failure because the gesture-to-image adapter did not exist. Added supported-file recognition, async storage-file reading, clipboard bitmap PNG encoding, and image-only default question behavior. Narrow result: 4/4 passed.
- Removed `AttachButton` and its storage picker. The full conversation panel now accepts dropped PNG/JPEG/WebP/GIF files. `TextBox.PastingFromClipboard` now stages a clipboard image or image file, while preserving ordinary text paste by inserting clipboard text at the current selection.
- An image can be sent without accompanying text; the direct turn uses `What is in this image?`. The staged filename remains visible beside the input until send.
- Verification: full solution build succeeded with 0 warnings and 0 errors; all 1,597 tests across 21 assemblies passed; GUI suite passed 106/106. No binaries were installed and no service was restarted.
## 2026-09-01 — Codex — G19 remove local models from GUI direct chat (started)

- Steve rejected the augmented interpretation of subscription chat. Claimed `20987b45`: GUI direct chat must contain no Ollama-backed retrieval, planning, disclosure rewrite, or vision step.
- Production logs for the failed screenshot show local Ollama generation/planning calls, then `IVisionClient` blocked for 67.8 seconds and failed with `ResponseEnded`. The failure was therefore in the local vision stage, before subscription inference.
- Verified the installed Codex 0.151.0 app-server schema: `turn/start.input` supports `{ "type": "localImage", "path": "..." }` alongside text. Plan: route GUI chat to a direct `IFrontierChat` stream and pass the staged image through that native subscription input.

## 2026-09-01 — Codex — G19 direct subscription implementation and deployment blocker

- Changed the GUI direct request to `frontier: true, augmented: false`; the TDD request test first failed because `frontier` was absent, then passed 2/2. Removed the misleading `retrieving locally` status text.
- Added the direct frontier branch ahead of both augmented and `ITurnRunner` branches. Text uses `IFrontierChat.StreamAsync` directly. Image bytes are written to a unique temporary file, supplied to Codex app-server as `{ type: "localImage", path }`, and deleted in `finally`; `IVisionClient` is not involved.
- Added app-server wire coverage for the installed schema and made failed subscription turns throw their reported error instead of silently ending with an empty stream. Provider app-server tests passed 5/5.
- Full build succeeded with 0 warnings/errors; all 1,597 tests across 21 assemblies passed. `git diff --check` passed.
- Installed the Release GUI successfully to `/home/steve/.local/opt/dami-gui`. Release Host publish succeeded to `/home/steve/.cache/dami-pub/host-g19`, but stopping/syncing/restarting `/opt/dami/host` failed before changing the live installation because sudo requires an interactive password. The live Host therefore remains the old binary and G19 is not marked complete.
## 2026-09-01 — Codex — G20 silent direct status and multiple images (started)

- Added and claimed `cd868f06`. Steve does not want provider-routing status printed in conversation, and image staging must be an ordered collection rather than a single replaceable slot.
- Plan: TDD the empty pending metadata and multi-image JSON/wire shapes, then change GUI staging, Host turn binding, frontier contract, temporary image lifecycle, and Codex app-server input together.

## 2026-09-01 — Codex — G20 silent direct status, multiple images, and persona (completed)

- GUI TDD first failed because `Images` and the empty presentation contract did not exist. Replaced the single `Image` property with an ordered `Images` array and made pending reply metadata empty. Drag/drop and clipboard file batches append every supported image; subsequent pastes/drops continue appending until send.
- Provider coverage initially failed because `CodexAppServer.TurnInput` accepted one path. Changed the frontier and app-server seams to ordered image collections, writes each to a unique temporary file, supplies all `localImage` items after the text item, and deletes every file afterward.
- Steve clarified Dami's personal identity. Added a failing frontier-identity test, then made the direct subscription voice explicitly identify Dami as a brilliant, warm, playful/flirty thirty-something Korean American woman, with an instruction not to force or stereotype the trait. The Host still prepends this voice directly; no local model is involved.
- Verification: full build succeeded with 0 warnings/errors; all 1,599 tests across 21 assemblies passed; `git diff --check` passed. GUI Release installed successfully. Latest Host Release staged at `/home/steve/.cache/dami-pub/host-g20`; live Host installation still requires Steve's interactive sudo authorization.
## 2026-09-01 — Codex — G21 composer image thumbnails (started)

- Added and claimed `e418d981`. The comma-separated filename ribbon does not provide useful visual confirmation. Plan: represent each staged request with a decoded thumbnail view model, bind an ordered horizontal strip inside the composer, and clear the strip atomically on send.

## 2026-09-01 — Codex — G21 composer image thumbnails (completed)

- Added `PendingChatImageTests`; observed the expected compile failure for the absent staged-image view model. Added a view model retaining the transport request and lazily decoding its preview on the Avalonia UI thread; the lazy boundary is required because headless tests have no render interface.
- Replaced the filename `TextBlock` with an ordered `ItemsControl` inside the dark composer border. Each image renders as a 58×58 `UniformToFill` thumbnail with rounded clipping, a subtle border, and filename tooltip; overflow scrolls horizontally. The text field and actions remain below the strip.
- `WindowState.PendingImages` is the single collection backing both previews and outgoing request order. Send projects requests from it and then clears the same observable collection, so visual and transport state cannot diverge.
- Verification: full build succeeded with 0 warnings/errors; all 1,600 tests across 21 assemblies passed; GUI suite passed 108/108; `git diff --check` passed. Release GUI installed to `/home/steve/.local/opt/dami-gui`; the running window must be reopened.
## 2026-09-01 — Codex — G22 inline composer and Shift+Enter (started)

- Added and claimed `cb6fb113`. The thumbnail tray remains a separate layout region; Steve wants image blocks and editable text in one CMS-like wrapping surface. Plain Enter must send while Shift+Enter remains an editor newline.
- Plan: TDD the keyboard decision, replace the bound tray with one `WrapPanel` whose generated thumbnail controls precede the live `TextBox`, and clear both view-model and visual blocks on send.

## 2026-09-01 — Codex — G22 inline composer and Shift+Enter (completed)

- Added `ComposerKeyTests`; observed the expected compile failure for the absent keyboard policy. Added the policy so plain Enter sends and Shift+Enter is not handled by send logic. Narrow result: 2/2 passed.
- Replaced the separate thumbnail row with one `WrapPanel` inside the composer. Each staged image inserts a thumbnail control immediately before the live `TextBox`; blocks and editor wrap together as space is consumed. The editor now accepts returns, wraps text, and grows to a bounded 120px height.
- Send clears both `WindowState.PendingImages` and every generated thumbnail control while retaining the editor control. Full build: 0 warnings/errors. Full suite: 1,602/1,602 passed; GUI: 110/110. `git diff --check` passed. Release GUI installed; reopen the running window to load it.
## 2026-09-01 — Codex — G23 multiline composer Enter routing (started)

- Added and claimed `bee70393`. Avalonia's multiline `TextBox` consumes Enter before a normally subscribed bubbling handler sees it, leaving neither send nor newline behavior reliable.
- Plan: TDD selection-aware newline insertion, register the key handler in the tunnel phase with handled events included, and explicitly handle both Enter outcomes.

## 2026-09-01 — Codex — G23 multiline composer Enter routing (completed)

- Added selection-aware newline cases to `ComposerKeyTests`; observed the expected compile failure for absent insertion behavior, then implemented caret insertion and selection replacement. Narrow composer-key suite passed 4/4.
- Replaced the ordinary bubbling `KeyDown` subscription with an Avalonia tunnel-phase handler registered with `handledEventsToo`. It now sees Enter before the multiline `TextBox` consumes it, marks it handled exactly once, dispatches send for plain Enter, and explicitly inserts a newline for Shift+Enter.
- Verification: full build succeeded with 0 warnings/errors; all 1,604 tests across 21 assemblies passed; GUI suite passed 112/112; `git diff --check` passed. Release GUI installed to `/home/steve/.local/opt/dami-gui`; reopen the running window.

## 2026-09-01 — Codex — G24 Dami image gallery and identity-preserving generation (started)

- Read the supplied recreation handoff, core decisions, novelty-ledger guidance, pose/expression guidance, and scheduled-persona skill. Inspected the supplied canonical and delivered portraits; they establish one fictional adult Dami identity, portrait 1024×1536 output, and deliberately varied narrative/action geometry.
- Added and claimed board task `49415686`. Existing code has a text-only `/images/generate` endpoint and an off-by-default three-times-daily portrait writer, but no canonical reference support, gallery index/provenance store, or Gallery UI.
- Plan: use strict narrow-test-first increments to add a persistent gallery catalog, import copies of the supplied artifacts, extend the hosted OpenAI image request with exactly one configured canonical reference using the provider edit endpoint, and add an Avalonia Gallery tab for browsing, viewing, and generating. No local image model or silent provider fallback will be introduced.

## 2026-09-01 — Codex — G24 Dami image gallery and identity-preserving generation (completed)

- OpenAI's current official documentation confirms `gpt-image-2` supports image input/output and `/v1/images/edits`; medium is the quality setting, not part of the model identifier. Added the reference-image provider test first, observed the missing contract compile failure, then implemented a singular `ImageReference` and multipart `image[]` edit request. Existing text-only generation stays on `/v1/images/generations`.
- Added gallery import/storage and generator tests before their production types. The store copies seeds, resolves only safe basenames, persists generated PNGs with JSON prompt/model provenance, reloads sidecars after a fresh service instance, and recovers historical timestamps from the supplied filenames. `GalleryImageGenerator` loads exactly one configured canonical anchor, composes the stable Dami identity/anatomy/novelty brief with the user's scene, requests `gpt-image-2` at medium quality and 1024×1536, and saves the result. No local image provider or fallback exists on this path.
- Added `/gallery`, `/gallery/{fileName}`, and `/gallery/generate`; configured the explicit supplied seed list and the approved `20260726_140042` identity anchor. Added the Avalonia Gallery tab with wrapping 188×250 portrait cards, selected large view, date/provider badges, refresh, and an in-tab scene composer. GUI metadata mapping was test-first.
- The first artifact-copy attempt failed without changing any originals because `/home/steve/.local/share/dami` is root-owned and cannot accept a new child. Moved the configured durable gallery to `/home/steve/Data/dami-gallery`, copied all 11 requested PNGs mode 0600, and verified the canonical source/destination SHA-256 values are identical (`3619e61f…`).
- Verification: solution build succeeded with 0 warnings/errors; Provider suite passed 55/55, Host suite 92/92, GUI suite 113/113, and all 21 suites passed 1,553 tests; `git diff --check` passed. Installed the Release GUI to `/home/steve/.local/opt/dami-gui`. Published the matching Host to `/home/steve/.cache/dami-pub/host-g24`; the root-owned live Host was not replaced or restarted because interactive sudo authorization is still required.

## 2026-09-01 — Codex — G25 unified image requests, gallery import, and global status (started)

- Live diagnosis: `/gallery` responds and the G24 Host is running, but journal evidence for the failed click says `OpenAiImageGenerator` refused `api.openai.com` because it is not allowlisted. The systemd production contract also explicitly says no API key exists anywhere by design; allowlisting would therefore only expose the next refusal. The G24 API-key image route was the wrong provider mechanism for this host.
- Added and claimed board task `6863d330`. Steve also added a multi-image Gallery import requirement.
- Plan: TDD a subscription-backed `IImageGenerator` that invokes Codex's authenticated built-in image-generation tool with no API key/local fallback; extend natural chat image intent to distinguish Dami self-portraits; add byte-preserving batch import APIs plus Gallery selection/drop UI; and centralize immediate accepted/busy/success/failure feedback in a persistent top-right status surface.

## 2026-09-01 — Codex — G25 superseded; G26 subscription images, Gallery import, and global status (completed)

- The first narrow provider test failed because `CodexSubscriptionImageGenerator` did not
  exist. Implemented it behind `IImageGenerator`: it invokes the logged-in `codex exec`
  built-in image tool, passes at most one identity reference, requires an Egressable request,
  copies its PNG result into the existing governed path, and has no API-key or local-model
  fallback. A second narrow test deliberately caught the prompt after `--image` argument
  ordering bug; placing the prompt before the greedy image argument made the test pass.
- Added and ran failing-first tests for Dami/self/surprise image-intent recognition,
  persistent status state, and byte-preserving multi-file Gallery import. Added the Gallery
  picker/drop target, immediate button busy state, and status messages for load, generation,
  import, success, failure, and cancellation. The direct chat route recognizes natural
  "picture of yourself" and "surprise me with what you are doing" language and calls the
  identity-preserving Gallery route, rendering the finished PNG inline.
- Full verification: `dotnet build Dami/Dami.sln --no-restore -v:q` finished with 0 warnings
  and 0 errors; `dotnet test Dami/Dami.sln --no-build --no-restore --logger
  "console;verbosity=quiet"` passed all 21 assemblies (1,561 tests, 0 failures); and
  `git diff --check` passed. `tools/install-gui.sh` installed the Release client to
  `/home/steve/.local/opt/dami-gui`.
- Published the Host Release to `/home/steve/.cache/dami-pub/host`, synced it to
  `/opt/dami/host`, restarted `dami-host`, and observed `active` plus
  `GET /health -> {"status":"ok"}`. The service remains owned and restarted by `steve`;
  no root configuration or credentials were changed.
- Live evidence: a direct subscription-tool validation produced
  `/tmp/dami-subscription-validation.png` (2,002,988 bytes). The first Gallery-import
  probe failed before reaching Dami because `jq` is not installed; a direct JSON stream then
  imported the PNG successfully and the live `/gallery` list reports it as `model: imported`.
  A live `POST /gallery/generate` then completed through the deployed subscription provider,
  saved `dami-20260902-012503-215b164c57bd4b03b7574849937eeda6.png`, and returned a valid
  1024×1536 2,181,030-byte PNG. Pixel inspection confirmed a coherent, identity-consistent
  Dami kitchen portrait offering coffee.
- The original G25 criterion requiring an `api.openai.com` allowlist was false for this
  deliberately API-key-free production architecture. Cancelled G25 rather than claim it as
  satisfied, added/claimed accurate replacement G26 `a1269cf2`, marked all five evidence-
  backed criteria satisfied, then completed G26 with the build, test, installed-GUI, live
  import, and live-portrait evidence.
- Post-deploy direct-chat proof: `POST /turns/stream` with the GUI's exact
  `frontier: true, augmented: false, images: []` shape returned incremental SSE fragments
  `SUB` / `S` / `CRIPTION` / … / `ATED` for `SUBSCRIPTION-ROUTE-VALIDATED` in 4.2 s.
  This is the deployed subscription stream, not the local turn runner or local vision path.

## 2026-09-01 — Codex — N9 Elasticsearch/Kibana observability and structured runtime logs (started)

- Claimed board task `f3947c38` after confirming the running inference containers and
  that no existing observability stack or compose manifest owns their names, ports, or
  data paths. The prior N3 task already owns the structured-template analyzer policy;
  N9 is the separate runtime logging and local search/dashboard stack.
- Plan: add a failing Host test for privacy-safe request/trace-correlated structured
  logging, configure both bare-metal .NET entry points for JSON console records, and
  provision pinned loopback-only Elasticsearch, Kibana, and a journal shipper with
  durable state under `/home/steve/Data/dami-observability`. Verify an actual Dami
  runtime record is searchable before declaring the Kibana criterion complete.

## 2026-09-01 — Codex — N10 disabled daily portrait proactive recovery (started)

- Restarting `dami-proactive` as part of the N9 release exposed an existing live defect:
  `DailyPortraitService` declares the newly introduced `EightHourly` cadence even while
  `DailyPortrait:Enabled` is false. The scheduler records every registered service, and
  migration 035's `proactive_runs_cadence_known` check permits only Nightly, Weekly, and
  Quarterly. PostgreSQL correctly rejected the row and the background-service host
  stopped. Claimed `d7a87242` to restore the tier without silently weakening the database
  check or enabling a metered portrait job.
- Plan: test that a disabled portrait option keeps the service out of proactive
  composition, register it only when explicitly enabled, verify the affected suite, then
  redeploy/restart the proactive tier and record its active state.

## 2026-09-01 — Codex — N9 Elasticsearch/Kibana observability and structured runtime logs (completed)

- Added `tools/observability/docker-compose.yml` and `filebeat.yml`: pinned Elasticsearch,
  Kibana, and Filebeat 9.5.2. Elasticsearch data is durable at
  `/home/steve/Data/dami-observability/elasticsearch`; Elasticsearch and Kibana publish
  only loopback ports. Filebeat tails only `dami-host.service` and
  `dami-proactive.service`, decodes their JSON console records, and retains a journal
  cursor in the named Docker volume. `tools/observability/README.md` documents the local
  security boundary and lifecycle.
- Added JSON console logging to both .NET entry points, request/activity correlation
  scopes, a low-noise request policy, and `StructuredRequestLoggingMiddleware`. Scope
  fields exclude request bodies and query strings. Direct frontier streams now set an
  `X-Dami-Trace` response header, so a client-visible trace can be matched to the
  structured request record. Tests were added before their respective production seams:
  the absent scope/policy types and absent streaming header first failed, then the narrow
  suite passed.
- Deployed the current Host/Proactive releases, created Kibana data view `Dami Runtime`
  (`dami-runtime-*`, `@timestamp`), and verified decoded Dami journal documents in
  Elasticsearch. `docker compose -f tools/observability/docker-compose.yml ps` reported
  Elasticsearch healthy and Kibana/Filebeat up; the published ports are
  `127.0.0.1:9200` and `127.0.0.1:5601` only.
- Verification: final `dotnet build Dami/Dami.sln --no-restore -v:q` succeeded with 0
  warnings and 0 errors. The full solution test command began after that build; all
  affected suites observed green: Host 101/101 and Proactive 245/245. `dami-host` health
  returned `{\"status\":\"ok\"}`.

## 2026-09-01 — Codex — N10 disabled daily portrait proactive recovery (completed)

- Added `Disabled_DailyPortrait_Should_Not_Be_Scheduled`; it first failed because the
  disabled service remained in composition. `ProactiveComposition` now registers the
  portrait service, its image generator, and its options only when
  `DailyPortrait:Enabled` is explicitly true. The narrow test then passed, and the full
  Proactive suite passed 245/245.
- After publishing/restarting the proactive tier, `systemctl is-active dami-host
  dami-proactive` returned `active` for both. The current proactive tick completed the
  embedder pass normally. The earlier `EightHourly`/`proactive_runs_cadence_known`
  PostgreSQL failure remains in the historical journal before this deployment; it no
  longer takes down the disabled tier. Enabling daily portraits later still needs its own
  cadence-migration task; this repair deliberately did not weaken the database check.

## 2026-09-01 — Codex — N9 direct-ingestion correction and handoff

- Steve correctly rejected Filebeat as the final Dami logging path: the two .NET hosts
  already expose `ILogger`, so structured records should use a direct Serilog Elasticsearch
  sink which becomes inert when Elasticsearch is unavailable. No Serilog package or
  direct-sink production change was made before the correction; Filebeat remains running
  temporarily only so the verified local stack is not torn down before its replacement.
- Added `Handoff.md` with the active deployment, source/board state, exact Filebeat
  removal boundary, direct Serilog replacement requirements, known test evidence, and
  dirty-worktree protections for the successor. N9 is not complete until the direct sink
  has replaced Filebeat and fallback behavior is demonstrated.
- Ran the repository-required ownership correction. New handoff/status/work-log files are
  `steve:steve`; four existing generated `obj/` cache files refused ownership changes and
  were left untouched.

## 2026-09-02 — Codex — Gallery failure diagnosis (started)

- The user reported that the Gallery is not working and requested diagnosis. Read
  `docs/onboarding.md`, inspected the shared dirty worktree without modifying Gallery
  sources, and attempted the required task-board lookup. The lookup could not run because
  `dami-host` refused the board client's connection on `127.0.0.1:5810`; no task was
  claimed.
- Plan: inspect deployed Host/GUI process state, local Gallery endpoints, structured
  journals, configuration/storage permissions, and the current Gallery request path;
  reproduce the narrowest failure and report the evidence-backed cause without changing
  production behavior unless separately requested.

## 2026-09-02 — Codex — Gallery failure diagnosis (completed)

- `systemctl status dami-host` reported inactive since 12:00:25 CDT; direct `/health`
  access and the board client both received connection refused. The Gallery is therefore
  failing because its shared loopback runtime is down, not because the Gallery route or
  artifacts disappeared.
- The journal establishes the failure chain: a Discord turn entered the subscription
  stream at 11:50:25.575 after disclosure processing, then `CodexAppServer.ReadDeltasAsync`
  threw `OperationCanceledException` at 12:00:25.581. That 600.006-second interval matches
  `CodexOptions.TimeoutSeconds = 600`. `DiscordGatewayWorker` catches cancellation only
  when the host stopping token is already cancelled and excludes every other cancellation
  from its general failure/fallback handler, so the per-turn deadline escaped
  `BackgroundService.ExecuteAsync`. The generic host then stopped by its default
  `BackgroundServiceExceptionBehavior`.
- The systemd unit has `Restart=on-failure`, but the generic host's graceful shutdown
  produced exit status 0; systemd consequently left it inactive instead of restarting it.
  No service state was changed during this diagnostic session.
- Gallery storage is intact and readable by `steve`: the directory is mode 0700 and owned
  by `steve:steve`, 72 image files remain, `file` recognized every checked asset as a
  supported image, and `json_pp` parsed all 61 sidecars. An attempted validation using
  `jq` and then `ruby`/`node` could not run because those executables are not installed;
  the successful `json_pp` result supersedes those tool failures.
- No production source or tests were changed. The board remained unavailable because it
  is served through the stopped Host, so no diagnosis task could be claimed or completed
  there.

## 2026-09-02 — Codex — Contain Discord/Codex timeout and restore Gallery (started)

- The user authorized fixing the crash potential and restarting the Host. The task board
  is still unavailable with the Host down, so work cannot be claimed before the repair;
  a board record will be created and closed after verified service restoration, with this
  outage recorded as the ordering deviation.
- Existing uncommitted Discord streaming and Codex image-input work overlaps the affected
  files. Inspected those diffs and will preserve them. Plan: add one failing regression at
  a time for frontier-timeout fallback, top-level per-message cancellation containment,
  and timed-out app-server reset; make the minimum production changes; run narrow and
  affected suites; publish the existing shared tree only after tests pass; restart the
  Host; then prove `/health`, `/gallery`, and the board are responsive.
- Added `Should_Fall_Back_To_The_Local_Model_When_The_Frontier_Times_Out` before changing
  production code. The first sandboxed test attempt aborted because VSTest could not open
  its local IPC socket. Re-running outside that restriction produced the expected red:
  0 passed / 1 failed with `OperationCanceledException: frontier deadline elapsed`
  escaping through `DiscordGatewayWorker.ExecuteAsync`.
- Added `Should_Survive_A_Canceled_Turn_When_The_Host_Is_Not_Stopping` before the
  top-level containment change. Its first run produced the expected red, 0 passed / 1
  failed, with the synthetic turn deadline escaping `ExecuteAsync`.
- Added `A_Timed_Out_Turn_Stops_The_App_Server_Process` before changing subprocess
  cleanup. The first two attempts exposed test-only compile/analyzer issues in its fake
  executable fixture; after correcting those without touching production, the behavioral
  run produced the intended red: 0 passed / 1 failed because the timed-out fake app-server
  process was still running.

## 2026-09-02 — Codex — N11 contain Discord/Codex timeout and restore Gallery (completed)

- `DiscordGatewayWorker.TryFrontierAsync` now distinguishes a component deadline from
  host shutdown and degrades that deadline to the existing visible local-model fallback.
  `ListenAsync` also contains any other `OperationCanceledException` when the Host stopping
  token is not cancelled, preserving the long-lived gateway. The two new regressions each
  failed first with the synthetic cancellation escaping `ExecuteAsync`, then passed after
  their respective production changes.
- `CodexAppServer` now treats every incomplete streamed turn as a poisoned protocol
  session: before releasing its one-turn semaphore it kills, awaits, disposes, and clears
  that subprocess. The Linux fake-process regression failed first because the timed-out
  process remained alive, then passed after cleanup was added.
- Verification: Discord Host suite passed 66/66, Providers passed 57/57, and Host passed
  101/101. The Release solution built with 0 warnings and 0 errors; all 21 assemblies
  passed 1,573/1,573 tests. `git diff --check` passed for all affected source, test, and
  documentation files.
- The first publish attempt ran inside the restricted sandbox, produced an empty staging
  directory without a useful error, and the first rsync therefore changed no deployed
  assemblies. The post-restart hash check caught that `/opt` still contained the September
  1 binaries. Re-running `dotnet publish` outside the sandbox populated the stage; the
  corrected Host-only rsync and restart then deployed matching SHA-256 values for
  `Dami.Host.Discord.dll` (`94d7b6ad…`) and `Dami.Providers.dll` (`f50a24f8…`). Proactive
  and CLI deployments were not touched.
- Final live proof under PID 1702318: `dami-host` is active, `/health` returned 200 with
  `{"status":"ok"}`, `/gallery` returned 200 with 72 items, and fetching
  `dami-20260902-013945-69216c43f2af400dabc978b5369dd61f.png` returned a valid
  1024×1536 PNG of 2,437,181 bytes. The board was unavailable until this restart; once
  restored, added/claimed N11 `d47a96d3`, marked all three criteria satisfied, and recorded
  the outage-driven ordering deviation rather than pretending the task was claimed first.
- Completed board task N11 `d47a96d3` with the red-green, 1,573-test, deployed-hash, and
  live-endpoint evidence. No commit or push was performed.

## 2026-09-04 — Claude — Discord never answers locally; the daily portrait actually fires (ADR-0028, ADR-0029)

Steve: "this agent is STILL using local model from discord IT SHOULD NEVER DO THAT, and
the daily cron image job is STILL not firing."

**Discord — what actually happened.** `journalctl -u dami-host` has exactly two local
answers since the 09-02 deploy, both on 2026-09-03: at 23:04:23 `DiscordRest.EditMessageAsync`
got a 429 on a progressive edit of the reply the frontier was *already streaming*; the
exception left the streamer, was caught as "frontier failed", and the finished frontier
answer was replaced by a qwen3 one. At 23:19:40 the Codex app-server produced no delta for
the full 600 s deadline (previous turn finished 23:05:17) and the deadline was caught as
"frontier timed out" — then `POST 127.0.0.1:11434/api/generate` and `/api/chat`. Both are
the fallback ADR-0026 kept and N11 widened. Removed it structurally: the worker no longer
takes `ITracedTurnRunner` (pinned by `Should_Have_No_Local_Model_To_Answer_With` and by the
composition test building without one), `DiscordAnswer.FrontierUnavailable` is the only
alternative to a frontier answer, nothing is journaled for an unanswered turn, and
`DiscordOptions.Frontier` is gone because with no local path it could only mean silence.
`EditMessageAsync` now shares the post path's Retry-After wait
(`EditMessageAsync_Should_Wait_And_Retry_When_Rate_Limited`). The ten-minute Codex hang is
**not** fixed here; it is now reported instead of hidden.

**Portrait — what actually happened.** It had never run. `dami.proactive_runs` has no
`daily-portrait` row, `dami.scheduled_jobs` is empty, `~/.local/share/dami/portraits` does
not exist, and `systemctl cat dami-proactive` has no `DailyPortrait__` line. The pass was
off by default (ADR-0027) and wired to `OpenAiImageGenerator`, which needs a key that was
never configured, while the Host had since moved portraits to
`CodexSubscriptionImageGenerator` + the identity anchor + the Gallery. Rewired the
proactive tier to the same generator (`Enabled_DailyPortrait_Should_Draw_On_The_Subscription_Not_A_Key`),
added `DailyPortrait:ReferencePath` and a shared `PortraitIdentity.PROMPT` in Contracts
(the Gallery composer now uses the same constant), and the pass writes the Gallery's
five-field sidecar beside the PNG. A configured-but-missing anchor is reported, not drawn
around.

**Gate.** `dotnet build Dami.sln`: 0 warnings, 0 errors. `dotnet test Dami.sln`: 21
assemblies, 1,576 passed, 0 failed. Discord host suite 63/63, gateway 60/60, proactive
250/250.

**Proof run, and the trap it sprang.** Ran the staged `Dami.Host.Proactive --run
daily-portrait` as steve with the four drop-in values in the environment. `codex exec`
drew `dami-2026-09-04-evening.png` — a valid 1024×1536 PNG, 2,064,638 bytes — into the
Gallery with its sidecar, 19:56 CDT. The process then aborted (exit 134) writing the run
row: SQLSTATE 23514, `proactive_runs_cadence_known` from migration 035 lists only
Nightly/Weekly/Quarterly. Handoff.md had named exactly this and N10 closed only the
disabled case; enabling the pass in the hosted tier would have stopped the tier on its
first pass. Added `tools/ddl/038_proactive_run_cadence_eight_hourly.sql`, registered it in
`TestDdl`, added a theory recording one run per `ProactiveCadence` value plus an enum-count
guard, and applied 038 to `dami-data` with `tools/ddl/apply.sh` (as `dami_ddl`, no sudo).
Re-run: "evening portrait already exists", exit 0, and `proactive_runs` now holds
`daily-portrait | 2026-09-04 20:00:23 | Completed | EightHourly`. Gate after the
migration: 0 warnings, 0 errors, **1,581 passed** (Persistence 283 → 288).

**Not done, and why.** Release builds of Host and Proactive are published to
`~/.cache/dami-pub/{host,proactive}` (Discord dll `640727ee…`, Proactive dll `6ab39eeb…`).
The rsync into `/opt`, the proactive drop-in lines, and the restarts were refused by this
session's permission layer and need `sudo` in any case (runbook §4.8). The board task
N12 could not be added for the same reason — the `dami board add` was refused. No commit.

## 2026-09-04 — Claude — Discord recognises "let's see an image of you" (deployed gap, live report)

Steve, 20:29 on Discord: "Let's see an image of you painting your toes" → Dami described a
pedicure and attached nothing; "Well where's the image?" → "I don't see an image".
Journal: both turns `answered by the frontier` — `DiscordImageResponder` matched only four
literal prefixes (`/image`, `create/generate/draw an image of`), so the request went to
the Codex text turn, whose own imagegen skill produced prose and no attachment.

Built `DiscordImageIntent` (pure, regex-backed, 15 cases): a picture *of you/yourself/
Dami* or a *selfie* is a portrait — scene rewritten ("you painting your toes" → "Dami
painting her toes") and routed through a new `IDiscordPortraitGenerator` seam that the
Host satisfies with `DiscordGalleryPortraits`: the Gallery generator with the identity
anchor, saved to the Gallery first and read back for the attachment, so Discord and the
Gallery tab show the same file. Any other picture request (noun + request cue, "picture
of X" subject extraction) uses the plain generator as before. A message carrying an
attachment, or talking about a picture Steve sent, is never a request. The worker test
replays the exact 20:20 message end to end.

Gate: 0 warnings, 0 errors; **1,598 passed** across 21 assemblies (Host.Discord 63 → 79,
Host 101 → 102). Host Release republished to `~/.cache/dami-pub/host` at 20:35; needs the
rsync + `systemctl restart dami-host`. Not fixed: Codex's text turn will still *say* it is
generating when a phrasing slips past the classifier — the frontier has an image tool of
its own and no way to attach here.

## 2026-09-04 — Claude — The frontier gets a tool bundle on Discord (ADR-0030)

Steve, after asking why Dami is worse than stock Hermes and hearing "Hermes lets the model
use tools; Dami's Discord path doesn't": "ok do it, give the frontier the tool bundle on
discord."

**Protocol.** `codex app-server generate-json-schema` / `generate-ts` for 0.152.1 show a
server→client request `item/tool/call` (`DynamicToolCallParams`: callId, tool, arguments)
and `DynamicToolCallResponse` (`success`, `contentItems[{type:"inputText",text}]`).
`thread/start.dynamicTools` is absent from the generated types; probing produced
`-32600 thread/start.dynamicTools requires experimentalApi capability`, and declaring
`capabilities.experimentalApi=true` on `initialize` unlocked it. Live probe at 20:45:
one declared `make_portrait`, prompt "make a picture of yourself painting your toes" →
tool call with a full scene → result accepted → "Here's a cozy portrait of me painting my
toes." Mechanism proven before any production code.

**Built.** Contracts: `FrontierTool/Call/Result`, `IFrontierToolHandler`,
`FrontierToolbox` (+`Empty`), `IFrontierChat.StreamAsync(prompt, images, tools)` with a
refusing default. Providers: `CodexAppServer` declares the capability, sends
`dynamicTools` when non-empty, answers `item/tool/call` on its id (handler exceptions →
`success:false`, never silence); `CodexChatClient` hosts the overload. Core:
`IAugmentedTurn.StreamAsync(question, context, tools)`; `AugmentedFrontierTurn` passes
the bundle through. Discord: `DiscordToolbox` builds a per-turn bundle of `make_portrait`
(Gallery generator, identity anchor) and `make_image` (plain generator), collects the
attachments; the worker sends them after the streamed text. The regex fast path from
earlier this evening stays in front.

**Tests.** App-server wire shapes pinned (capability, `dynamicTools` function spec,
response shape) plus a Linux fake server that only completes the turn after reading the
reply on id 7 with `success:true`. `AugmentedFrontierTurnToolTests` (the class had no
direct tests before). `DiscordToolboxTests` ×6. Worker: bundle offered on every turn; a
frontier that calls the tool mid-stream gets its picture attached.
`EgressSeamTests.Image_Generator_Holders_Should_Be_The_Pinned_Set` failed as designed
and `DiscordToolbox` was added with the ADR reference.

**Gate.** 0 warnings, 0 errors; **1,613 passed** across 21 assemblies (Providers 57 → 62,
Core 234 → 236, Host.Discord 79 → 87). Host Release restaged 20:57; needs
`sudo rsync … /opt/dami/host/` and `systemctl restart dami-host`. No live Discord turn
with the deployed bundle yet — that is Steve's next message.

## 2026-09-04 — Claude — First live image request after deploy failed: the anchor lived in ~/Downloads

21:02 on Discord: "Let's see an image of you painting your toes" → "I couldn't create that
image." Journal: `InvalidOperationException: The canonical Dami identity image is not
configured` from `GalleryImageGenerator.ReferenceAsync`. `ImageGallery:CanonicalReferencePath`
in `appsettings.json` pointed at `/home/steve/Downloads/openai_gpt-image-2-medium_20260726_140042_9808fb1e.png`,
which no longer exists (Downloads was cleaned); the Gallery's seeded copy at
`/home/steve/Data/dami-gallery/` is byte-identical and was sitting right there. The GUI's
Gallery generation would have failed the same way since the file went.

Fix: the generator falls back to the Gallery's own copy by file name when the configured
path is missing, and only fails — as `FileNotFoundException`, naming the path — when
neither exists; `appsettings.json` now points at the Gallery copy. Two tests. Gate: 0
warnings, 0 errors, **1,615 passed**. Host restaged 21:0x; `appsettings.json` in the
...
### 2026-09-16 — Claude — commit, deploy (pushed: no)

- Steve: "commit and push everything, build and deploy", then "the bootstrap HAS run". No `.cs` file had changed since the 2,105-test gate; `git add -A` and one commit, `49bf0db`, carrying the Gemini door, the code tools, the sudo-free deploy scripts, and the other agent's in-flight research/GUI/event-store work as it stood (Steve's word was "everything"). The post-commit board import ran (223 tasks; the three pre-existing G5a1/G5a2/O1 conflicts, left as they were).
- `git push origin main` was refused by this session's permission classifier ("Blocked by classifier"); not retried. main is 31 commits ahead of origin/main and has never been pushed from this clone. Steve runs the push.
- Bootstrap verified: `zz-user-config.conf` on both units, `~/.config/dami/{host,proactive}.env` 0600, `/etc/polkit-1/rules.d/49-dami-units.rules` installed 19:17 CDT. `tools/deploy.sh --no-build` at 19:19 CDT from this (non-root, no password) shell: gate build clean and tests green, 47 migrations applied and none pending, rsync into `/opt/dami`, allowlist already complete, `systemctl --no-ask-password restart dami-host dami-proactive` succeeded, `dami-host dami-proactive dami-tts` all active, `/health` 200, TTS health ok, zero error-level journal lines in either unit in the three minutes after. `/opt/dami/host/Dami.Providers.dll` contains `CodexCodeWorker` and `GeminiImageGenerator`.
- Both env files hold only their header comments: `Images__Provider`, `GeminiImages__ApiKey` and `CodeWork__Enabled` are unset, so nothing behaves differently yet. The runbook sections have the lines.
- Steve: "set them then." Appended to `~/.config/dami/host.env`: `CodeWork__Enabled=true`, `Egress__AllowedHosts__0=generativelanguage.googleapis.com`, and the `Images__Provider=Gemini` / `GeminiImages__ApiKey` pair **commented** (a provider with no key refuses every image; the key is Steve's to paste). `proactive.env` got `Egress__AllowedHosts__10=…` and the same commented pair. Restarted both units at 19:24 CDT without sudo; `dami-host`/`dami-proactive` active, `/health` 200, `/proc/<pid>/environ` of the live Host shows `CodeWork__Enabled=true` and the allowlist line, zero error-level journal lines. The code tools are live on the next frontier turn; Gemini waits on the key.

### 2026-09-16 — Claude — Gemini as the backup image door (started)

- Steve: "keep codex as the primary, anytime a image request is refused use gemini as a backup the key is in ~/dev/dami-agent/Dami/gemini-key.txt", then "if the image doesn't generate from the primary you retry it using gemini" and, on the no-secrets rule, "I wrote it which means I can break it." The key was untracked (54 bytes, one line, not matched by `*.key`); moved to `~/.config/dami/gemini-key.txt` (0600) and written into `GeminiImages__ApiKey` in both env files via `sed` from the file, never echoed; `gemini-key.txt` and `*-key.txt` added to `.gitignore`. `Images__Backup=Gemini` set in both env files; `Images__Provider` left unset (Codex).
- TDD: `FallbackImageGeneratorTests` (10) and two `ProactiveCompositionTests` written first; RED was CS0246 `FallbackImageGenerator`. Wrote `FallbackImageGenerator` (primary, then backup on any non-cancellation refusal or failure, except a non-Egressable prompt, which stays refused — D-012), `ImageProviderOptions.Backup`, `ImageProviderRegistration.AddImageGenerator(provider, backup)` registering concrete doors and the pair, both composition roots, and the `EgressSeamTests` pin. One compile slip (`PrivacyClass` lives in `Dami.Contracts.Context`). GREEN: Providers 132/132, Proactive 279/279, Architecture 20/20, Host 122/122; `dotnet format --verify-no-changes` on the eight changed files exit 0. Full gate running.
- Full gate before the deploy: 0 warnings, 0 errors, 2,117 passed / 0 failed in 21 assemblies (`gate3-*.log`). Committed `fc368ca`; `tools/deploy.sh` (full: gate, publish, rsync, restart) landed at 19:44 CDT, `/health` 200.
- **Fallback drill, 19:44:51 CDT.** Appended `Codex__ImageTimeoutSeconds=1` to `host.env`, restarted the Host, `POST /images/generate` with a plain prompt. Ledger, in order: `image-codex-subscription EgressRequested 19:44:51` → `image-gemini EgressRequested 19:44:52` → `image-gemini EgressFailed HttpRequestException 19:44:52`; the endpoint answered 500 with "429 (Too Many Requests)" in 1.4 s. The retry path works end to end; the backup itself was refused by Google. Removed the override and restarted at once (`/proc/<pid>/environ` shows no `Codex__ImageTimeoutSeconds`); Codex is primary again and had drawn the 19:35–19:38 Gallery portrait normally.
- Direct probe with the key (no Dami code): `generateContent` on `gemini-3.1-flash-image` and `gemini-2.5-flash-image` both 429 `RESOURCE_EXHAUSTED` — "Quota exceeded for metric: generativelanguage.googleapis.com/generate_content_free_tier_requests, limit: 0". The key is on the free tier, where image output has a quota of zero; the key's Google Cloud project needs a billing account linked. `GET /v1beta/models` with the key lists six image models, so the key itself is valid. Responses saved in the session scratchpad, not in the repository.
- Two gaps the drill exposed, fixed TDD: (1) `GeminiImageGenerator` threw a bare `HttpRequestException` for the status; it now reads the error body and throws `Gemini answered 429 TooManyRequests: RESOURCE_EXHAUSTED: <Google's message>` (`Should_Say_What_The_Provider_Said_When_It_Refuses`, passed first run after the implementation; it was written with the fix). (2) `CodexSubscriptionImageGenerator` left the 19:44:51 `EgressRequested` with no outcome; RED `Should_Record_EgressFailed_When_The_Tool_Dies`, then the catch that emits `EgressFailed` (body moved to `DrawAsync` to keep the method under the line limit). Providers 134/134; format clean on the four files.

### 2026-09-16 — Claude — Elasticsearch triage and why the proactive tier feels like it does nothing (started)

- Steve: "check our elasticsearch logs to see if there are any errors and then I would like for this agent to be more intelligently pro-active. As it is now it doesn't seem to do shit except create images and chat."
- ES, last 7 days (`dami-runtime-*`): 9 `Error` records, 511 `Warning`. Errors: 8 `StructuredRequestLoggingMiddleware` request failures on `dami-host` (5 `OperationCanceledException`, 3 `InvalidOperationException` at 2026-09-12 02:34Z, 1 `HttpRequestException` at 2026-09-17 00:44Z while the Codex image door returned no file) and 1 `ProactivePassRunner` failure (`gallery-curator`, 2026-09-10, Ollama "response ended prematurely" while `dami-llm-guard` restarted the sidecar). Warnings: 315 slow-request lines, 94 Discord gateway reconnects (62 "closed 0 no close frame", 30 "connection failed", 10–26 per day), 58 `CuratorService` failures all at 19:25:11 CDT tonight (same second `dami-llm-guard` restarted `dami-llm`; the curator kept going through its whole batch), 14 deep-research skips on 2026-09-13 (favicons/PDF and the 30-per-hour egress budget tripping on RFC-editor crawling), 4 image-tool throws (Codex "returned no image file"), 2 disclosure-gate withholdings.
- Proactive tier, last 14 days (`proactive_runs`, `surfacings`, `conclusions`): 21 services, 287 runs, 2 failed; 173 surfacings delivered in 30 days, 16 rated. Feed composition: interest-scout 65 (avg confidence 0.55), daily-portrait 28, civic-agenda 22, repo-hygiene 17 (one rated `bad`), cve-watch 9. **The conclusions ledger holds 3 active rows, all from 2026-08-23/24; no proactive pass has added a belief since.** `reflection` ran twice (09-06, 09-13), both "the model proposed nothing".
- Root cause, with receipts: `docker logs dami-llm` shows 122 `truncating input prompt` warnings in 14 days, every one `limit=2050 keep=4`. The 2026-09-13 17:19:56Z line is the reflection pass: `prompt=332874` tokens truncated to 2050 — the instructions were the first thing dropped, so "proposed nothing" was literal. `OllamaChatClient` never sends `num_ctx` (the vision client does, 8192); the model supports 40,960. The prompt was that large because `chat` observations average 5,539 chars (max 13,272) — `TurnRunner.RecordInteractionAsync` bounds the answer to 240 chars but records the request in full, and the GUI Network tab sends its whole collector dump as the request ("Steve is looking at the Network tab…" ×39 in that week, 480k chars). Discord turns are not recorded into the corpus at all (`DiscordAnswerer` has no corpus dependency; only the `remember` tool writes, 7 rows in 30 days).
- Plan (board H19, `0fc1f95c`): `OllamaOptions.ContextTokens` + `num_ctx` on both `OllamaChatClient` paths; `ReflectionOptions.MaximumObservationCharacters`/`MaximumPromptCharacters` so the prompt is bounded and the instructions come first; `TurnRunner` bounds the recorded request; `CuratorService` stops its pass on the first `HttpRequestException` instead of failing every remaining note. Not in scope without Steve: recording Discord turns into the corpus, and pushing surfacings rather than waiting for his next message (ADR-0014 is still proposed).

### 2026-09-16 — Claude — H19 built (not deployed, not committed)

- TDD, RED first: `OllamaChatClientTests` (2 new, CS0117 `ContextTokens`), `ReflectionServiceTests` (3 new, CS1061 `MaximumObservationCharacters`/`MaximumPromptCharacters`), `CuratorServiceTests` (new file, 4), `TurnRunnerTests.RunAsync_Should_Bound_The_Request_It_Records` (assertion failure). Then production: `OllamaOptions.ContextTokens` = 12288 and `num_ctx` on both `OllamaChatClient` request objects; `ReflectionOptions` gains the two bounds, `ReflectionService.BuildPrompt` lists observations until the budget and trims the list so provenance numbers stay exact, and logs the model's reply when it proposes nothing; `TurnRunner.RecordInteractionAsync` bounds the request to 400 chars; `CuratorService.CurateAllAsync` breaks on `HttpRequestException`. `DAMI0003` caught a 37-line method on the first cut; split.
- Gate from `Dami/`: `dotnet build Dami.sln` 0 warnings, 0 errors; `dotnet test Dami.sln --no-build` 2,129 passed / 0 failed / 0 skipped in 21 assemblies (`gate-build.log`, `gate-test.log` in the session scratchpad).
- Live, from the tree build (`dotnet Dami.Host.Proactive.dll --run <service>` with the service's connection string): `--run reflection` at 20:11 CDT → "2 observation(s) since 09/13 17:19:56Z; below the floor of 3, staying quiet" (its LastRanAt is now 09-16 20:11, so the next weekly pass is due 09-23). `--run curator` 20:12–20:17 CDT → "rewrote 55 of 60"; `docker logs dami-llm` shows `n_ctx = 12288` once (the reload) and `n_ctx_slot = 12288` on all 61 requests, no `truncating input prompt`. VRAM after: llama-server 6,880 MiB (was 5,616 at 4,096), TEI 2×1,408 MiB.
- Docs: status.md Phase 4 row "Reflection can read its own prompt (H19)", runbook trap §4.9, TODO.md H19 from the board export. Board `0fc1f95c` stays claimed: criterion 1 (a reflection pass observed at the new window) is not yet met — the pass had nothing to read.
- Left for Steve: deploy (`tools/deploy.sh`), commit, and the two decisions the numbers point at — Discord turns do not reach the corpus at all (`DiscordAnswerer` has no `IObservationCorpus`; 40 Discord turns in 3 days produced 2 corpus rows), and surfacings are pull-only (five "noticed" lines on his next message; ADR-0014 still proposed). Neither is fixable by a bigger context window.

### 2026-09-16 — Claude — proactive addon catalog (research, no code)

- Steve: "investigate some of the most popular plugins/addons for other personal assistant/agents and curate a list that would be fun, useful, insightful etc to implement in our agent."
- Three parallel web surveys (open-source registries; MCP/commercial connectors; praised-and-disliked proactive features), each with per-claim URLs and explicit "unverified" marks. Curated against D-012/D-020/D-021, the domain inventory, and what already runs. Written to `docs/proactive-addon-catalog.md`: 21 items in three tiers, a stop-doing list with Dami's own surfacing numbers, and a suggested first three (lessons ledger, correlation card + single-fact nudge, INR cadence). Nothing added to the board; the picks are Steve's.

### 2026-09-16 — Claude — H20 lessons ledger, H21 signals, H22 INR cadence (built, not deployed, not committed)

- Steve: "go for it" on the catalog's suggested first three. Board tasks added and claimed: H20 `1ef7994d`, H21 `b2484359`, H22 `6c46d20e`.
- TDD, RED first (CS0246 on `DailySeries`, `IDailySeriesSource`, `SignalsOptions`, `IFrontierLesson`, `StandingLessons`, `InrCadenceService`, `PostgresConversationActivitySource`; CS1501 on the 4-argument `DiscordPrompt.LocalContext`). New tests: `DailySeriesStatisticsTests` 6, `SignalNudgeServiceTests` 7, `CorrelationCardServiceTests` 6, `GymSeriesSourceTests` 3, `CommitsSeriesSourceTests` 2, `InrCadenceServiceTests` 6, `LessonToolTests` 3, `StandingLessonsTests` 4, `PostgresConversationActivitySourceTests` 2 (DB-backed, cleans its own 2031 rows), plus one line in `DiscordPromptTests`, two in `FrontierToolBundleTests` (the bundle is seventeen tools now), and the three names in `ProactiveCompositionTests`.
- Production: `Dami.Contracts.Domains.{DailyPoint,DailySeries,IDailySeriesSource}`; `Dami.Proactive.Signals.{SignalsOptions,DailySeriesStatistics,SignalNudgeService,CorrelationCardService,GymSeriesSource,CommitsSeriesSource}`; `IGitLog.CommitTimesAsync` + `GitProcessLog`; `Dami.Persistence.Domains.PostgresConversationActivitySource` (registered in `AddDamiPersistence`); `Dami.Proactive.Health.{InrCadenceOptions,InrCadenceService}`; `Dami.Core.Frontier.{LessonTool,StandingLessons}`; `FrontierToolBundle` takes `IFrontierLesson`; `DiscordAnswerer` takes `IStandingLessons` and `DiscordPrompt.LocalContext` puts lessons first; registrations in Host `Program.cs` and `ProactiveComposition.AddSignals`. `DAMI0003` (30-line methods) fired four times along the way; each split.
- Live from the tree build (`--run`): `inr-cadence` → "0 valued check(s) on the timeline; below 3, staying quiet" — the 16 INR/warfarin rows in `health_events` are prose without values, so H22 is quiet by data until a check is logged. `signal-nudge` → "5 series examined for 09/15/2026; nothing was a high or low". `correlation-card` first run surfaced gym volume vs working sets at r = 0.96 — same-source arithmetic, a defect: the card now pairs only across sources (test added), and that pending row `4eecfbe2…` was deleted as `dami_ddl` (`dami_app` has no delete, by design). Second run surfaced "On days with more conversations between midnight and 5am, commits to dami-agent tends to be higher (r = 0.50 over 84 days)" — a real cross-source pair, exactly at the floor; the phrasing was then changed for plural labels.
- Gate from `Dami/`: `dotnet build Dami.sln` 0 warnings, 0 errors; `dotnet test Dami.sln --no-build` 21 assemblies, 2171 passed, 0 failed, 0 skipped (`gate4-*.log` in the session scratchpad).
- Docs: status.md three Phase 4 rows; runbook section "the signals, lessons and INR passes"; TODO.md H20–H22 from the board export; the catalog header notes the three are built. Board criteria stay open: H20 needs a deploy and a live correction on Discord; H21's nightly half has not yet had an extreme day to show; H22 has no data to lapse.
- Not done, deliberately: the GUI's direct chat bypasses local context (G19), so lessons reach Discord only; Discord turns still do not enter the corpus (decision pending from the earlier entry); nothing deployed or committed.

### 2026-09-16 — Claude — G30 web view from LAN workstations (proxy live; host change not deployed; not committed)

- Steve: "we also need a web-based front end so I can test out the app from workstations on my network." Found: the web view at `127.0.0.1:5810/` already exists (single file, chat over `/turns/stream`, events, inbox, beliefs, approvals, health) but its chat sent only `{message}` — the local-model path; `Program.cs` hardcoded `UseUrls("http://127.0.0.1:5810")`; OIDC (ADR-0020) is behind `Authentication:Enabled` and not cut over (G5a4 open, Codex), and with it on the fallback policy would require auth on every path the page fetches without a token; `/etc/ufw/ufw.conf` says `ENABLED=no`; sshd listens on 0.0.0.0:22.
- Built, TDD: `HostUrlsTests` (6, RED on CS0103) → `Dami.Host.HostUrls.Resolve`: loopback default, `Host:Urls` honoured, any non-loopback URL refused with a message naming D-005/ADR-0020 unless `Authentication:Enabled`. `Program.cs` uses it. Web view: a `chatmode` selector (frontier + tools default, gated memory, local) and the request body carries `frontier`/`augmented`. `tools/lan-proxy/{docker-compose.yml,Caddyfile,set-password.sh,README.md}`: `caddy:2.11.4` on the host network, `https://<LAN address>:8443` + loopback names with `tls internal`, `basic_auth` from `LAN_PROXY_USER`/`LAN_PROXY_HASH`, `reverse_proxy 127.0.0.1:5810 { flush_interval -1 }`, `/lan-proxy/health` unauthenticated for the healthcheck. `tools/dami-up`/`dami-down` include the container (`curl -skf` for the https readiness URL).
- Three defects found while bringing it up: `caddy hash-password` on piped stdin needs the trailing newline (EOF otherwise); a port-only site block gets no certificate from the internal CA (TLS "internal error" until the site was named with the addresses); Compose interpolates `$` in `env_file` values, which ate the bcrypt hash (`$2a$14.`) — the script now writes `$$`.
- Verified: local `https://127.0.0.1:8443/` 401 without the credential, 200 (30,133 B index) with it; `/health` and `/events?after=0` 200 with it. From a bridge container to `https://192.168.4.45:8443/` (crosses the host INPUT chain): health 200, `/` 401 unauthenticated, `/` and `/events` 200 with the credential. Certificate SAN `IP Address:192.168.4.45`, issuer Caddy Local Authority. The random password is in `~/.config/dami/lan-proxy.password` (0600); never printed here. Container `dami-lan-proxy` is running (healthy).
- Gate from `Dami/`: 0 warnings, 0 errors; 2,177 passed / 0 failed / 0 skipped in 21 assemblies (`gate5-*.log`).
- Board `e6c94753` claimed; criterion "refuses a non-loopback URL … shown by a test" marked yes; the other (opened from a real second machine with a streamed frontier turn) waits on Steve and on a deploy of the page change. Not deployed, not committed. Runbook §"Reaching the web view from another workstation", status.md web view row, TODO.md G30, `tools/lan-proxy/README.md`.

### 2026-09-16 — Claude — M1f: the four-portrait turn that ended in "did not answer within its deadline" (built, not deployed, not committed)

- Steve pasted the Discord transcript (21:52–22:02 CDT, trace `5caf2a25`). Evidence: the augmented turn's events are under `16d250e5` (its own id); the tool bundle's id `5caf2a25` has 0 events. Portrait egress spans `141c6344` 21:52:39→21:54:42, `95a48994` 21:54:48→21:57:35, `94f82c20` 21:57:45→22:00:29 (each ~3 min on the Codex image tool, 1.99–2.03 MB), `e019fef3` requested 22:00:36 and never completed. `DiscordAnswerer` logged "Frontier turn timed out; nothing was answered" at 22:02:15 — `CodexOptions.TimeoutSeconds` (600) from 21:52:14, with tool time counted. No "answered … picture(s)" line and no multipart send for that turn: the three finished portraits are in `gallery_images` (02:54:42, 02:57:35, 03:00:29Z) and never reached Discord, because attachments are sent only after a successful stream.
- Three defects, TDD: (1) `CodexAppServerTests.Tool_Time_Should_Not_Count_Against_The_Turn_Deadline` (fake server, 2 s tool under a 1 s budget) → `TurnBudget` in `CodexAppServer` pauses the deadline around `tools.Handler.HandleAsync` and re-arms with what is left; (2) `AugmentedFrontierTurnToolTests.A_Caller_Supplied_Trace_Should_Be_The_Turns_Trace` → `IAugmentedTurn.StreamAsync(question, localContext, tools, traceId, ct)`; `DiscordAnswerer` passes its own id, so the error line's trace is the one with events (`DiscordGatewayWorkerTests.Should_Give_The_Error_Message_The_Trace_That_Has_The_Events`); (3) `DiscordGatewayWorkerTests.Should_Deliver_The_Pictures_It_Finished_When_The_Turn_Times_Out` → `DiscordAnswerer` owns the turn's tools and `ExplainAsync` attaches `tools.Pictures` with "The N picture(s) it had finished are attached." on timeout or failure. 21 stubs in the Discord tests moved to the five-argument `StreamAsync`; the hand-written fake in `TaskWorkServiceTests` gained the overload. `DAMI0003` fired three times; split.
- Gate from `Dami/`: 0 warnings, 0 errors; 2,181 passed / 0 failed / 0 skipped in 21 assemblies (`gate6-*.log`).
- Board `53915955` (M1f under M1) claimed; its criterion needs a live multi-portrait turn after a deploy. status.md acceptance row 11 and TODO.md updated. Not changed: the 600 s model budget and the 480 s per-image ceiling; four portraits still take ~12 minutes, they just finish now.

### 2026-09-16 — Claude — commit, push, deploy (pushed: yes)

- Steve: "commit, push, deploy". Six commits staged by path (shared files split by hunk): `ed925c6` H19, `d3c2c6e` catalog, `7d0d574` H20–H22, `f5cc75b` G30, `d5b6b04` M1f, `7909b34` docs. No attribution trailers. The post-commit board import reported three pre-existing conflicts (G5a1/G5a2 Done on the board, O1 InProgress on the board; TODO.md says otherwise) and left the board as it is. `git push origin main` 5684f3b..7909b34 — the first push from this clone; origin had nothing new.
- `tools/deploy.sh` 23:05:51–23:06:51 CDT, exit 0: gate 0 warnings / 0 errors, 2,181 passed; schema up to date (47); `/opt/dami/{cli,host,proactive}` 23:06; `dami-host`, `dami-proactive`, `dami-tts` active; `/health` 200; the deployed page carries `id="chatmode"`; `https://127.0.0.1:8443/` 200 with the credential; 0 error lines in either journal in the three minutes after the restart.
- Now live: num_ctx 12,288 on every local call, the bounded reflection prompt, the lesson tool and standing lessons on Discord, signal-nudge / correlation-card / inr-cadence in the proactive tier, the frontier-default web chat, the HostUrls guard, the paused turn budget and picture delivery on failure. Still waiting: H19's criterion (the 09-23 reflection), G30's (a second machine), M1f's (a multi-portrait turn), H20's (a correction on Discord).

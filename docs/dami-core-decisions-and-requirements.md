...
Prepared for: Steve Hoff Date: 2026-08-22 Companion documents: Dami-Core-Project-Charter.md, Dami-Core-Architecture.md
...
Payload serialization inside the transport frame — MemoryPack, hand-rolled span writer, other. Framing must not depend on it.
...
Backup destinations, encryption, retention schedule
Repository visibility and licensing
Which Hermes sessions and skills are worth migrating at all
Whether the Mac remains permanently as an Apple-services bridge
Remote API exposure beyond localhost, and its auth design
Instrumentation results — TTFT vs stream duration vs tool round-trips on Hermes and MAI. Determines whether the latency win comes from prompt weight or round-trip count.

...
public sealed class DiscordAnswerer
{
    ...
    public async Task AnswerAsync(InboundMessage message, string question, CancellationToken cancellationToken)
    {
        ArgumentNullException.ThrowIfNull(message);
        var captions = await this.vision.DescribeAsync(message, cancellationToken).ConfigureAwait(false);
        await this.AnswerAsync(message.ConversationId, question, captions, cancellationToken).ConfigureAwait(false);
    }

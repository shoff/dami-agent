...
- [x] A8 **Deploys without sudo** (Claude 2026-09-16) — `tools/bootstrap-sudoless-deploy.sh` (once, as root) installs a polkit rule scoped to `dami-*` units and steve-owned `~/.config/dami/{host,proactive}.env` read last by systemd; `tools/deploy.sh`, `dami-up`, `dami-down` no longer call sudo. Scripts syntax-checked; rule exercised in a JS harness. Bootstrap not yet run `[STEVE: sudo bash tools/bootstrap-sudoless-deploy.sh, once]`
  - criterion: `tools/deploy.sh` completes from an SSH session with no password prompt and the restarted units answer /health
...
- [x] D7 **Gemini image generation provider** (Claude 2026-09-16, ADR-0035) — `GeminiImageGenerator` in `Dami.Providers`, a keyed HTTP door gated like the OpenAI one (Egressable-only, allowlisted host, absent key = absent capability, C5 budget, every call in the event stream) against `models/{model}:generateContent` with an image-only response and the reference as an inline part. `Images__Provider` (`Codex` default · `OpenAi` · `Gemini`) selects the one `IImageGenerator` both tiers register through `AddImageGenerator`. 26 provider tests + 2 composition tests; full gate 0 warnings, 0 errors, 2,061 tests. `dami board add` was refused (409, parent not resolved), so recorded here for the next import. Not verified live — no Gemini key on this host `[STEVE: key out of band, drop-in lines in runbook §"Choosing the image provider"]`
  - criterion: the first real Gemini call is recorded in the runbook with its `image-gemini` event rows
- [x] D8 **The frontier changes its own code on a branch** (Claude 2026-09-16, ADR-0036) — `change_code` / `explain_code` / `list_code_changes` in the frontier bundle behind `CodeWork__Enabled` (off): a task becomes a `git worktree` on `dami/<stamp>-<slug>`, `codex exec --sandbox workspace-write` there, this host's own `dotnet build`, a commit on the branch; never the main tree, never merged/pushed/deployed. `ICodeWorker` seam, `CodexCodeWorker` door, codex-process holders pinned in `EgressSeamTests`. 38 new tests; full gate 2,105 green. Not run live `[STEVE: CodeWork__Enabled=true in ~/.config/dami/host.env, runbook §"Letting Dami change her own code"]`
  - criterion: the first real `dami/` branch produced from Discord is recorded in the runbook with its execution_events rows
